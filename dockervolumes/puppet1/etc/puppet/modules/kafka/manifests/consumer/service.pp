...
#  }
  ...
  service { 'supervisor-kafka':
    ensure     => running,
    enable     => true,
    hasstatus  => true,
    hasrestart => true,
    start    => "supervisorctl -c /etc/supervisor/supervisord_database.conf start kafka",
    stop     => "supervisorctl -c /etc/supervisor/supervisord_database.conf stop kafka",
    restart  => "supervisorctl -c /etc/supervisor/supervisord_database.conf restart kafka",
    pattern  => "kafka",
