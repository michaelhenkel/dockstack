class dockstack::gal (
  ...
  $hostname = $::hostname
  notify { "reg galstered_galera : $registered_galera":; }
  ...
  $mysql_vip_root_user = join(['root@',$vip_fqdn],'')
  $mysql_vip_root_grant = join(['root@',$vip_fqdn,'/*.*'],'')
  $mysql_password_hash = mysql_password($galera_password)
  mysql_user { $mysql_vip_root_user:
    ensure                   => 'present',
    password_hash	     => $mysql_password_hash,
  }
  mysql_grant { $mysql_vip_root_grant:
    ensure     => 'present',
    options    => ['GRANT'],
    privileges => ['ALL'],
    table      => '*.*',
    user       => $mysql_vip_root_user,
  }
  mysql::db { 'keystone':
    user     => 'keystone',
